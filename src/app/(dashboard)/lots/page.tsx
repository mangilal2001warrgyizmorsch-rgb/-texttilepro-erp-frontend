...
import { Plus, Layers, Filter, Search, List, LayoutGrid, Eye, Trash2, ChevronRight, MoreVertical } from "lucide-react";
...
export default function LotsPage() {
  ...
  const { data: lotResponse, isLoading } = useQuery({
    ...
    queryFn: () => statusFilter === "Pending" 
      ...
      : api.get<any[]>(`/lots${statusFilter ? `?status=${statusFilter}` : ""}`),
  });

  const rawLots = (lotResponse as any)?.data || (Array.isArray(lotResponse) ? lotResponse : []);
  ...
  const lots = rawLots.filter((lot: any) => {
    ...
    return (
      ...
      (lot.partyName || lot.firm || lot.firmName || "").toLowerCase().includes(term) ||
      ...
      <div className="p-6 pb-2 shrink-0">
        <div className="flex items-center justify-between mb-4">
          <div>
            ...
            <p className="text-sm text-muted-foreground">Manage and track production batches across the facility</p>
          </div>
          <Button 
        ...
        <div className="flex flex-wrap items-center gap-3 bg-card border p-3 rounded-xl shadow-sm mb-4">
          ...
          <div className="flex items-center gap-2 border-l pl-3 border-border/50">
            {(["All", "Pending", "InStorage", "InProcess", "Finished"] as const).map((s) => (
              <Button
                key={s}
                ...
                className={cn(
                  ...
                  statusFilter === (s === "All" ? undefined : s) ? "shadow-md bg-primary" : "text-muted-foreground hover:bg-muted"
                )}
              >
                {s === "Pending" ? "Awaiting Lot" : s === "InStorage" ? "In Storage" : s === "InProcess" ? "In Process" : s}
              </Button>
            ))}
          </div>
        ...
        {isLoading ? (
          ...
          <Empty className="py-20 bg-card border-2 border-dashed rounded-3xl">
            <EmptyHeader>
              ...
              <Button onClick={() => setStatusFilter(undefined)} variant="link" className="text-primary font-bold">Clear Filters</Button>
            </EmptyHeader>
          </Empty>
        ) : viewMode === "grid" ? (
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4">
            {lots.map((item: any) => {
              ...
              return (
                <div key={item._id} className="bg-card border border-border/60 rounded-xl p-4 hover:shadow-md transition-all group flex flex-col gap-3">
                  <div className="flex items-start justify-between">
                    <div>
                      <div className="flex items-center gap-2 mb-1">
                        <span className="font-bold text-lg">{isChallan ? (item.challan_no || item.challanNo) : item.lotNo}</span>
                        <Badge variant="outline" className={cn("text-[10px] font-bold uppercase py-0 px-2 rounded-md", STATUS_COLORS[item.status] || "bg-muted")}>
                          {item.status}
                        </Badge>
                      </div>
                      <p className="text-xs font-semibold text-muted-foreground truncate max-w-[150px]">{isChallan ? (item.firm || item.firmName) : item.partyName}</p>
                    </div>
                    {isChallan ? (
                      <Link href={`/lots/new?challanId=${item._id}`}>
                        <Button size="icon" className="rounded-full h-8 w-8 shadow-sm">
                          <Plus size={16} />
                        </Button>
                      </Link>
                    ) : (
                  ...
                  <div className="grid grid-cols-2 gap-2 text-[11px] pt-2 border-t border-border/40">
                    <div>
                      <p className="text-muted-foreground font-medium uppercase tracking-tighter">Quality</p>
                      <p className="font-bold truncate">{item.qualityName || item.quality || "N/A"}</p>
                    </div>
                    <div className="text-right">
                      <p className="text-muted-foreground font-medium uppercase tracking-tighter">Total Meter</p>
                      <p className="font-bold text-primary">{(item.meter || item.totalMeter || 0).toFixed(1)}m</p>
                    </div>
                  </div>
                </div>
              );
