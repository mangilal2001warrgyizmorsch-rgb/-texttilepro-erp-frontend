...
function ChallanEntry() {
  ...
  const onPartyChange = (val: string) => {
    const p = masters.find(m => m.accountName === val);
    if (p) {
      updateField("party", p.accountName);
      updateField("party_address", p.address || "");
      updateField("gstin_no", p.gstin || "");
    } else {
      updateField("party", val);
    }
  };

  const onQualityChange = (val: string) => {
    const q = qualities?.find(x => x.qualityName === val);
    if (q) {
      updateField("quality", q.qualityName);
      updateField("hsn_code", q.hsnCode || "");
      updateField("width", q.width?.toString() || "");
    } else {
      updateField("quality", val);
    }
  };
  ...
  const handleFileUpload = async (file: File | null | undefined) => {
    ...
    try {
      ...
      const res = await fetch(`${process.env.NEXT_PUBLIC_API_URL || "http://localhost:5500/api"}/ocr/extract`, { 
        method: "POST", 
        body: formData,
        headers
      });
      ...
      if (data.challans && data.challans.length > 0) {
        const mapped = data.challans.map((c: any) => ({
          // Basic Details
          challan_no: String(c.challan_no || "").trim(),
          challan_date: formatDate(c.challan_date),
          date: formatDate(c.date),
          firm: String(c.firm || "").trim(),
          party: String(c.party || "").trim(),
          party_address: String(c.party_address || "").trim(),
          gstin_no: String(c.gstin_no || "").trim(),
          
          // Item & Quality
          quality: String(c.quality || "").trim(),
          hsn_code: String(c.hsn_code || "").trim(),
          item: String(c.item || "").trim(),
          taka: String(c.taka || "").trim(),
          meter: String(c.meter || "").trim(),
          dyed_print: String(c.dyed_print || "").trim(),
          weaver: String(c.weaver || "").trim(),
          
          // Rates & Amounts
          fas_rate: String(c.fas_rate || "").trim(),
          amount: String(c.amount || "").trim(),
          weight: String(c.weight ?? "").trim(),
          total: (() => {
            const w = String(c.weight ?? "").trim();
            const a = String(c.amount || "").trim();
            return (!w || Number(w) === 0) ? a : "";
          })(),
          chadhti: String(c.chadhti || "").trim(),
          width: String(c.width || "").trim(),
          pu_bill_no: String(c.pu_bill_no || "").trim(),
          
          // Dispatch Details
          lr_no: String(c.lr_no || "").trim(),
          lr_date: formatDate(c.lr_date),
          transpoter: String(c.transpoter || "").trim(),
          remark: String(c.remark || "").trim(),
          
          // Taka Details Table
          table: Array.isArray(c.table) 
            ? c.table.map((row: any) => ({
                tn: row.tn || 0,
                meter: String(row.meter || "").trim(),
              }))
            : [],
        }));
        ...
        toast.error("No challans found in PDF. Please check the file.");
      }
    } catch (e: any) {
      console.error("PDF extraction error:", e);
      toast.error(e.message || "Failed to extract PDF. Please try again.");
    } finally {
      setLoading(false);
    }
  };
  ...
  const formatDate = (d: any) => {
    ...
    // Handle DD/MM/YYYY format (from PDF)
  ...
  const updateField = (field: string, value: string) => {
    setChallans((prev) =>
      prev.map((c, i) => {
        ...
        // Auto-calculate total: if weight is 0 or empty, total = amount; else total = ""
        if (field !== "total") {
          const weight = field === "weight" ? value : updated.weight;
          const amount = field === "amount" ? value : updated.amount;
          const weightNum = Number(weight);
          if (!weight || weightNum === 0 || isNaN(weightNum)) {
  ...
  const handleSubmit = async () => {
    try {
      setLoading(true);
      await api.post("/challans/batch", { challans });
    ...
    <div className="space-y-4 flex flex-col flex-1 overflow-hidden min-h-0">
      ...
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 shrink-0">
        ...
        <div className="flex flex-wrap gap-2">
          ...
          <Button className="cursor-pointer h-9 bg-primary text-primary-foreground hover:opacity-90" onClick={handleSubmit} disabled={loading}>
          ...
          <div className="flex-1 overflow-y-auto p-4 md:p-6 space-y-6 custom-scrollbar">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div className="space-y-1.5">
                  ...
                  <Select value={form.firm} onValueChange={(v) => updateField("firm", v)}>
                    ...
                    <SelectContent>
                      {mills.map(m => (
                        <SelectItem key={m._id} value={m.accountName}>{m.accountName}</SelectItem>
                      ))}
                    </SelectContent>
                  </Select>
                </div>
                <div className="space-y-1.5">
                  ...
                  <Select value={form.party} onValueChange={onPartyChange}>
            ...
            <div className="space-y-4">
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                <div className="space-y-1.5">
                  ...
                  <Select value={form.quality} onValueChange={onQualityChange}>
                    <SelectTrigger className="h-9"><SelectValue placeholder="Select Quality" /></SelectTrigger>
                    <SelectContent>
                      {qualities?.map(q => (
                        <SelectItem key={q._id} value={q.qualityName}>{q.qualityName}</SelectItem>
                      ))}
                    </SelectContent>
                  </Select>
                </div>
              ...
              <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                ...
                <div className="space-y-1.5">
                  ...
                  <Select value={form.weaver} onValueChange={(v) => updateField("weaver", v)}>
                    ...
                    <SelectContent>
                      {weavers?.map(w => (
                        <SelectItem key={w._id} value={w.weaverName}>{w.weaverName}</SelectItem>
                      ))}
                    </SelectContent>
                  </Select>
                </div>
            ...
            <div className="space-y-4">
              ...
              <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                ...
                <div className="space-y-1.5">
                  <Label className="text-xs">Total <span className="text-muted-foreground">(auto)</span></Label>
                  <Input type="number" placeholder="0.000" className="h-9 bg-muted/30" value={form.total} readOnly />
                </div>
              </div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                ...
                <div className="space-y-1.5">
                  ...
                  <Select value={form.transpoter} onValueChange={(v) => updateField("transpoter", v)}>
                    ...
                    <SelectContent>
                      {transporters.map(t => (
                        <SelectItem key={t._id} value={t.accountName}>{t.accountName}</SelectItem>
                      ))}
                    </SelectContent>
                  </Select>
                </div>
...
function ChallanHistory() {
  ...
  const { data, isLoading } = useQuery({
    ...
    queryFn: () => api.get<any>(`/challans?page=${page}&limit=20${search ? `&search=${search}` : ""}`),
  });
  ...
  const handleDelete = async (c: any) => {
    if (!confirm(`Are you sure you want to delete challan #${c.challan_no}?`)) return;
    try {
  ...
  return (
    <div className="space-y-4">
                ...
                <tr key={c._id} className="hover:bg-muted/30 transition-colors">
                  ...
                  <td className="px-5 py-3 text-right">
                    <div className="flex items-center justify-end gap-1">
                      <Button size="icon" variant="ghost" className="h-7 w-7 text-muted-foreground hover:text-primary rounded-full cursor-pointer" onClick={() => setSelectedChallan(c)} title="View Details">
                        <Eye size={14} />
                      </Button>
                      <Button size="icon" variant="ghost" className="h-7 w-7 text-muted-foreground hover:text-blue-500 rounded-full cursor-pointer" onClick={() => setEditingChallan(c)} title="Edit Challan">
                        <Edit size={14} />
                      </Button>
                      <Button size="icon" variant="ghost" className="h-7 w-7 text-muted-foreground hover:text-red-500 rounded-full cursor-pointer" onClick={() => handleDelete(c)} title="Delete Challan">
                        <Trash2 size={14} />
                      </Button>
                    </div>
                  </td>
      ...
      {editingChallan && <ChallanEditDialog challan={editingChallan} onClose={() => setEditingChallan(null)} />}
    </div>
  );
}
...
function ChallanDetailDialog({ challan, onClose }: { challan: any; onClose: () => void }) {
  const { data: fullChallan, isLoading } = useQuery({
    ...
    queryFn: () => api.get<any>(`/challans/${challan._id}`),
  });
...
function ChallanEditDialog({ challan, onClose }: { challan: any; onClose: () => void }) {
  const qc = useQueryClient();
  const [loading, setLoading] = useState(false);
  ...
  const [form, setForm] = useState<any>({
    ...emptyForm(),
    ...challan,
    date: formatDateForInput(challan.date),
    challan_date: formatDateForInput(challan.challan_date),
    lr_date: formatDateForInput(challan.lr_date),
    table: challan.table || [],
  });
  ...
  const removeRow = (idx: number) => {
    setForm((prev: any) => ({
      ...prev,
      table: prev.table
        .filter((_: any, ri: number) => ri !== idx)
        .map((r: any, ri: number) => ({ ...r, tn: ri + 1 })),
    }));
  };
  ...
  const handleSubmit = async () => {
    try {
      setLoading(true);
      await api.put(`/challans/${challan._id}`, form);
      qc.invalidateQueries({ queryKey: ["challan"] });
      toast.success("Challan updated successfully!");
      onClose();
    } catch (e: any) {
      toast.error(e.message || "Failed to update challan");
    } finally {
      setLoading(false);
    }
  };
  ...
  return (
    <Dialog open={true} onOpenChange={onClose}>
      <DialogContent className="sm:max-w-6xl w-[95vw] max-h-[90vh] flex flex-col p-0 overflow-hidden bg-background">
        <DialogHeader className="px-6 py-4 border-b shrink-0 bg-card">
          <DialogTitle>Edit Challan: #{form.challan_no}</DialogTitle>
        </DialogHeader>
          ...
          <div className="lg:col-span-5 xl:col-span-4 h-[300px] lg:h-full flex flex-col bg-card border-l overflow-hidden">
            ...
            {form.table.length > 0 && (
              <div className="p-3 border-t bg-muted/10 shrink-0 flex items-center justify-between">
                <span className="text-xs font-bold uppercase tracking-wider text-muted-foreground">Total Meter</span>
                <span className="text-sm font-bold text-primary">
                  {form.table.reduce((s: number, r: any) => s + (parseFloat(r.meter) || 0), 0).toFixed(2)}
                </span>
              </div>
            )}
          </div>
        ...
        <div className="px-6 py-4 border-t flex justify-end gap-2 shrink-0 bg-muted/20">
          <Button variant="outline" onClick={onClose} disabled={loading} className="cursor-pointer">Cancel</Button>
          <Button onClick={handleSubmit} disabled={loading} className="cursor-pointer">Save Changes</Button>
        </div>
      </DialogContent>
    </Dialog>
  );
