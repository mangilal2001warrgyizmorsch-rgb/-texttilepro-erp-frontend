...
export default function NewChallanPage() {
  ...
  const { data: ordersResponse } = useQuery({
    ...
    queryFn: async () => {
      // If we have a specific ID, fetch it directly to ensure it exists in our local state
      if (orderId) {
        const res = await api.get<any>(`/orders/${orderId}`);
        // Handle both direct object and {data: object} responses
        const data = res?.data || res;
        return Array.isArray(data) ? data : [data];
      }
      // Otherwise fetch draft/pending orders for the selection dropdown
  ...
  // Sync selectedOrderId with URL parameter
  useEffect(() => {
    if (orderId) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setSubmitting(true);
    try {
      // Calculate total auto if not set
      ...
      const challan = await api.post("/challans", payload);
        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-muted-foreground text-sm">
              Issue a challan against a draft order
            </p>
          </div>
        </div>
      </div>
          ...
          <form onSubmit={handleSubmit} className="p-6 space-y-8">
            ...
            <div className="space-y-4">
              ...
              <div className="grid grid-cols-1 md:grid-cols-6 gap-4">
                ...
                <div className="space-y-1.5 md:col-span-3">
                  ...
                  <Select
                    key={selectedOrderId + "_firm"}
                    value={form.firm}
                    onValueChange={(v) => updateField("firm", v)}
                  >
                    ...
                    <SelectContent>
                      {mills.map((m) => (
                        <SelectItem key={m._id} value={m.accountName}>
                          {m.accountName}
                        </SelectItem>
                      ))}
                      {form.firm &&
                        !mills.some((m) => m.accountName === form.firm) && (
                          <SelectItem value={form.firm}>{form.firm}</SelectItem>
                        )}
                    </SelectContent>
                  </Select>
                </div>
                <div className="space-y-1.5 md:col-span-3">
                  ...
                  <Select
                    key={selectedOrderId + "_party"}
                    value={form.party}
                    onValueChange={(v) => updateField("party", v)}
                  >
                    <SelectTrigger className="h-9">
                    ...
                    <SelectContent>
                      {masterAccounts.map((p) => (
                        <SelectItem key={p._id} value={p.accountName}>
                          {p.accountName}
                        </SelectItem>
                      ))}
                      {form.party &&
                        !masterAccounts.some(
                          (p) => p.accountName === form.party,
                        ) && (
                          <SelectItem value={form.party}>
                            {form.party}
                          </SelectItem>
                        )}
                    </SelectContent>
                ...
                <div className="space-y-1.5 md:col-span-3">
                  ...
                  <Input
                    ...
                    value={form.gstin_no}
                    onChange={(e) => updateField("gstin_no", e.target.value)}
                  />
                </div>
                <div className="space-y-1.5 md:col-span-3">
              ...
              <div className="grid grid-cols-1 md:grid-cols-12 gap-4">
                <div className="space-y-1.5 md:col-span-4">
                  ...
                  >
                    ...
                    <SelectContent>
                      {qualities?.map((q: any) => (
                        <SelectItem key={q._id} value={q.qualityName}>
                          {q.qualityName}
                        </SelectItem>
                      ))}
                      {form.quality &&
                        !qualities?.some(
                          (q) => q.qualityName === form.quality,
                        ) && (
                          <SelectItem value={form.quality}>
                            {form.quality}
                          </SelectItem>
                        )}
                    </SelectContent>
                ...
                <div className="space-y-1.5 md:col-span-3">
                  <Label className="text-xs">
                    Taka <span className="text-red-500">*</span>
                  </Label>
                  <Input
                    type="number"
                    className="h-9"
                ...
                <div className="space-y-1.5 md:col-span-3">
                  ...
                  <Select
                    key={selectedOrderId + "_weaver"}
                    value={form.weaver}
                    onValueChange={(v) => updateField("weaver", v)}
                  >
                    <SelectTrigger className="h-9">
                    ...
                    <SelectContent>
                      {weavers?.map((w: any) => (
                        <SelectItem key={w._id} value={w.weaverName}>
                          {w.weaverName}
                        </SelectItem>
                      ))}
                      {form.weaver &&
                        !weavers?.some((w) => w.weaverName === form.weaver) && (
                          <SelectItem value={form.weaver}>
                            {form.weaver}
                          </SelectItem>
                        )}
                    </SelectContent>
              ...
              <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
                <div className="space-y-1.5">
                  ...
                  <Input
                    ...
                    placeholder="0.00"
                    value={form.fas_rate}
                    onChange={(e) => updateField("fas_rate", e.target.value)}
                  />
                </div>
                <div className="space-y-1.5">
                ...
                <div className="space-y-1.5">
                  <Label className="text-xs">
                    Total <span className="text-muted-foreground">(auto)</span>
                  </Label>
                  <Input
                    type="number"
                    step="0.001"
                ...
                <div className="space-y-1.5">
                  ...
                  <Input
                    ...
                    onChange={(e) => updateField("pu_bill_no", e.target.value)}
                  />
                </div>
              </div>
            </div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                <div className="space-y-1.5">
                  ...
                  <Input
                    ...
                    value={form.lr_no}
                    onChange={(e) => updateField("lr_no", e.target.value)}
                  />
                </div>
                <div className="space-y-1.5">
                  ...
                  <Input
                    ...
                    className="h-9"
                    value={form.lr_date}
                    onChange={(e) => updateField("lr_date", e.target.value)}
                  />
                </div>
                <div className="space-y-1.5 md:col-span-2">
                  <Label className="text-xs">Transporter</Label>
                  <Select
                    key={selectedOrderId + "_trans"}
                    value={form.transpoter}
                    onValueChange={(v) => updateField("transpoter", v)}
                  >
                    <SelectTrigger className="h-9">
                    ...
                    <SelectContent>
                      {transporters.map((t: any) => (
                        <SelectItem key={t._id} value={t.accountName}>
                          {t.accountName}
                        </SelectItem>
                      ))}
                      {form.transpoter &&
                        !transporters.some(
                          (t) => t.accountName === form.transpoter,
                        ) && (
                          <SelectItem value={form.transpoter}>
                            {form.transpoter}
                          </SelectItem>
                        )}
                    </SelectContent>
            ...
            <div className="pt-4 border-t flex justify-end">
              <Button
                type="submit"
                size="lg"
                className="bg-primary hover:opacity-90 gap-2 cursor-pointer shadow-sm"
                disabled={submitting}
              >
                <CheckCircle size={18} />
